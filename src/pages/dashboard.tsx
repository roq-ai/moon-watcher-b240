...
import { getMoonMovements } from 'apiSdk/moon-movements';
import { MoonMovementInterface } from 'interfaces/moon-movement';
import { getDataModels } from 'apiSdk/data-models';
import { DataModelInterface } from 'interfaces/data-model';
import { getDataAnalysts } from 'apiSdk/data-analysts';
import { DataAnalystInterface } from 'interfaces/data-analyst';

import { AstronomyExpertListPage as ShowCaseList } from 'pages/astronomy-experts';

function DashboardPage() {
  const { session } = useSession();

  const moonMovementStat = useSWR<PaginatedInterface<MoonMovementInterface>>(
    () => `/moon-movements`,
    () => getMoonMovements({ limit: 1, offset: 0 }),
  );

  const dataModelStat = useSWR<PaginatedInterface<DataModelInterface>>(
    () => `/data-models`,
    () => getDataModels({ limit: 1, offset: 0 }),
  );

  const dataAnalystStat = useSWR<PaginatedInterface<DataAnalystInterface>>(
    () => `/data-analysts`,
    () => getDataAnalysts({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Moon Movements',
      result: moonMovementStat?.data?.totalCount,
      isLoading: moonMovementStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Data Models',
      result: dataModelStat?.data?.totalCount,
      isLoading: dataModelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Data Analysts',
      result: dataAnalystStat?.data?.totalCount,
      isLoading: dataAnalystStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Astronomy Experts
                </Text>
                <NextLink href={`astronomy-experts`} passHref legacyBehavior>
                  <Button
