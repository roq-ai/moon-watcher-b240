...
import { getDataAnalystById } from 'apiSdk/data-analysts';
import { DataAnalystInterface } from 'interfaces/data-analyst';

function DataAnalystViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DataAnalystInterface>(
    () => (id ? `/data-analysts/${id}` : null),
    () =>
      getDataAnalystById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('data_analyst', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/data-analysts/edit/${id}`} passHref legacyBehavior>
                    <Button
